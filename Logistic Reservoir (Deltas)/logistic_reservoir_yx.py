...
def rc(m):
    ...
    # ax.plot(time[pred_i:], lz[pred_i:], label = 'Truth Values')
    # ax.plot(time[pred_i:], pred.flatten(), '--', label = f'Predicted Values; RMSE = {round(rmse, 4)}')
    ...
    # secax.set_xlabel('$t_{L} = t \cdot \lambda_{max}$ ($\lambda_{max} =  0.9056$)')
    # secax.tick_params(axis = 'x', direction= 'in', top= True)

    # ax.set_title(f'Logistic RC to Predict Lorenz z(t) from x(t); $\\delta = {delta}$; Memory: {m}; [3.5, 4.0]')
    # ax.set_xlabel('t')
    # ax.set_ylabel('z(t)')
    # ax.legend()
    # ax.grid(True)
    # plt.savefig('logistic_rc_results.png')
...
# Save to CSV
with open('logistic_error_vs_memory_yx.csv', 'w', newline='') as file:
